...
namespace UpdateNight.TocReader.Parsers
{
    public sealed class IoPackageReader : PackageReader
    {
        ...
        private void ReadContent()
        {
            ...
            for (int i = 0; i < referencedPackagesCount; i++)
            {
                ...
                string importedPackageName = Transform(Utils.GetFullPath(importedPackageId) ?? "");
                if (!(Utils.GetPropertyPakPackage(importedPackageName) is IoPackage package)) continue;
            ...
            foreach (var i in exportOrder)
            {
                ...
                catch (Exception e)
                {
                    ...
                    Console.WriteLine("Failed to read export {0} ({1}) of type {2}", exportMapEntry.ObjectName.String, i, exportType.String);
                    Console.WriteLine(e);
                }
        ...
        {
            if (Global.CachedSchemas.TryGetValue(export, out var v))
            {
                return v;
            }
            else
            {
                ...
                while (type != null)
                {
                    var schema = Global.Usmap.Schemas.FirstOrDefault(x => x.Name == type);
                    if (schema.Name != null)
                    {
                ...
                Global.CachedSchemas[export] = ret;
                return ret;
            }
        }
        ...
        private string Transform(string path)
        {
            if (path.StartsWith("/FortniteGame/Content"))
                return path.Replace("FortniteGame/Content", "Game");

            if (path.StartsWith("/FortniteGame/Plugins"))
            {
                ...
                string trigger = tPath[tPath.LastIndexOf('/')..];
